...
import Image from "next/image" // Still used for gallery, floor plans, etc.
...
export default function PropertyLandingPage() {
  ...
  if (!ready) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center bg-background text-foreground">
        ...
        <p className="mt-4 text-lg">Loading page translations...</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground antialiased overflow-x-hidden">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-card border-border shadow-xl">
                  ...
                  <DropdownMenuItem onClick={() => changeLanguage("en")} className="hover:bg-muted focus:bg-muted">
                    {t("langSwitchEN", "English")} {currentLanguage.startsWith("en") && "✓"}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => changeLanguage("pt")} className="hover:bg-muted focus:bg-muted">
                    {t("langSwitchPT", "Português")} {currentLanguage.startsWith("pt") && "✓"}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            ...
            >
              <div className="aspect-w-16 aspect-h-9 md:aspect-h-7 rounded-t-xl overflow-hidden">
                <iframe
                  ...
                  title={t("mapAltText", "Map showing property location in Aveiro")}
      ...
      <footer className="py-12 bg-foreground text-background/80 text-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-sm">
            &copy; {new Date().getFullYear()} Mirador Residence Aveiro.{" "}
            {t("footerText").split("© ")[1]?.split(" ")[1] || "All rights reserved."}
          </p>
        </div>
      </footer>
    </div>
